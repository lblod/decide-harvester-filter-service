...
async function extractAndInsertSubjectsForType(type, resultGraphUri) {
  ...
  const total = await fetchTargetCount(queryDefinition);
  ...
  while (offset < total) {
    ...
    await insertBatch(queryDefinition, resultGraphUri, BATCH_SIZE, offset);
    await sleep();
    offset += BATCH_SIZE;
  }
}

async function fetchTargetCount(queryDefinition) {
  const countQuery = buildCountQuery(queryDefinition);
...
async function insertBatch(queryDefinition, resultGraphUri, batchSize, offset) {
  const insertQuery = buildInsertWhereQuery(queryDefinition, resultGraphUri, batchSize, offset);
