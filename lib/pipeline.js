...
export async function run(deltaEntry) {
  ...
  try {
    ...
    const bestuursorganen = getBestuursorganen(task.resource);
    if (!bestuursorganen) {
      throw new Error(`No bestuursorganen configured for bestuurseenheid <${task.resource}>`);
    }
