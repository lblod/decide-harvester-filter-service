...
function buildValuesBlock(varName, bestuursorganen) {
  const lines = bestuursorganen.map((bestuursorgaan) => sparqlEscapeUri(bestuursorgaan)).join("\n");
...
export function buildCountQuery(queryDefinition, restrictionGraph, bestuursorganen) {
  const s = "?s";
  ...
  const pathBlock = path.length
    ? `${s} ${path} ?bestuursorgaan .\n${buildValuesBlock("?bestuursorgaan", bestuursorganen)}`
    : `${buildValuesBlock(s, bestuursorganen)}`;
