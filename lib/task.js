...
export async function loadTask(subject) {
  const queryTask = `
   ${PREFIXES}
   SELECT DISTINCT ?graph ?task ?inputContainerGraph ?id ?job ?created ?modified ?status ?index ?operation ?error ?resource WHERE {
     GRAPH ?graph {
  ...
  const queryStr = `
    ...
    INSERT DATA {
       GRAPH <${task.graph}> {
        ...
        ${sparqlEscapeUri(container.uri)} ext:hasResource ${sparqlEscapeUri(task.resource)}.
