    ...
    WHERE {
       GRAPH ?g {
        ?task a ${sparqlEscapeUri(TASK_TYPE)};
              ...
              task:operation ${sparqlEscapeUri(OPERATION_URI)}.
...
export async function loadTask(subject) {
  const queryTask = `
   ${PREFIXES}
   SELECT DISTINCT ?graph ?task ?inputContainer ?inputContainerGraph ?resource ?id ?job ?created ?modified ?status ?index ?operation ?error WHERE {
     GRAPH ?graph {
      ...
      ?inputContainer task:hasResource ?resource.

      OPTIONAL { ?inputContainer task:hasGraph ?inputContainerGraph. }
      OPTIONAL { ?task task:error ?error. }
     }
    }
  `;

  const rows = parseResult(await query(queryTask, connectionOptions));
  if (!rows.length) return null;

  const { graph, task, id, job, created, modified, status, index, operation, error } = rows[0];
  const inputContainers = [
    ...new Map(
      rows.map((row) => [
        row.inputContainer,
        { uri: row.inputContainer, resource: row.resource, graph: row.inputContainerGraph },
      ]),
    ).values(),
  ];

  return { graph, task, id, job, created, modified, status, index, operation, error, inputContainers };
}
...
export async function appendTaskResultGraph(task, container, graphUri, resource) {
  // prettier-ignore
  const queryStr = `
    PREFIX dct: <http://purl.org/dc/terms/>
    PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
    ...
    INSERT DATA {
       GRAPH ${sparqlEscapeUri(task.graph)} {
        ...
        ${sparqlEscapeUri(container.uri)} task:hasResource ${sparqlEscapeUri(resource)}.
